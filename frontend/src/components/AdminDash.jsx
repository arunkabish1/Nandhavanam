...
export default function AdminDash() {
  ...
  const handleGenerateEventPost = async () => {
    if (!eventData.event) {
      setEventStatus({
        message: "Please enter an event name first to generate a post.",
        type: "error",
      });
      return;
    }
  ...
  const handleGenerateMemberBio = async () => {
    if (!memberData.name || !memberData.position) {
      setMemberStatus({
        message: "Please enter a name and position first to generate a bio.",
        type: "error",
      });
      return;
    }
            ...
            <div className="bg-white p-6 rounded-lg shadow-md w-full lg:w-2/3">
              ...
              <form onSubmit={addEvent} className="space-y-4">
                ...
                <div className="flex flex-col">
                  ...
                  <textarea
                    className="border min-h-40 border-gray-300 p-2 rounded focus:ring-2 focus:ring-cyan-600 outline-none"
                    name="post"
                    id="post"
